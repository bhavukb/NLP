...
if __name__ == "__main__":
    ...
    # print(pred_data[0])
    test = np.array([[pred_data[i], gold_data[i], i] for i in range(len(gold_data))])
    test_filtered = filter(lambda x: (x[1]!='O'), test)
    test_filtered = list(filter(lambda x: (x[1]!='O' and x[0]==x[1]), test))
    print(test_filtered)
